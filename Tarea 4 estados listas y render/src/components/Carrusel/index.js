...
export default function Carrusel(props) {
	const [elResutlado, elError] = useConexion('genre/movie/list');
		...
		if (elResutlado != null) {
			elResutlado.genres.forEach((elGenero) => {	
				const elNuevoArreglo = {
					id: elGenero.id,elNombreDelGenero:elGenero.name, laLista: laLista.filter((elElemento) => {
						if (elElemento.genre_ids.
							some(elTituloPorGen => elTituloPorGen == elGenero.id))
							return elElemento
					})
				};
	...
	function EnlisteLosGeneros() { 	
		...
		losGeneros.forEach((elGenero,Key) => {
			if(elGenero.laLista.length > 0){
				elArrglo.push(<div key={elGenero.id+1 }>
					<h1 >{elGenero.elNombreDelGenero}</h1>
					<Categoria  elContenido={elGenero.laLista}></Categoria>;
				</div>)
				}
		})
